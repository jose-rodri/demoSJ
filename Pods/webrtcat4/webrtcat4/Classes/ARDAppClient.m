...
static NSInteger const kARDAppClientErrorSocketError = -7;
...
- (instancetype)init {
    return [self initWithDelegate:nil serverURL:@""];
}

- (instancetype)initWithDelegate:(id<ARDAppClientDelegate>)delegate serverURL:(NSString*)serverUrl {
    if (self = [super init]) {
        NSLog(@"URL base %@", serverUrl);

        _roomServerClient = [[ARDAppEngineClient alloc] init];
        _roomServerClient.baseURL = serverUrl;
        _kARDIceServerRequestUrl = [NSString stringWithFormat:@"%@/params", serverUrl] ;
        _delegate = delegate;
        NSURL *turnRequestURL = [NSURL URLWithString:self.kARDIceServerRequestUrl];
        _turnClient = [[ARDTURNClient alloc] initWithURL:turnRequestURL];
        NSLog(@"URL turn client %@", turnRequestURL);
...
- (void)connectToRoomWithId:(NSString *)roomId
    ...
    NSLog(@"connectToRoomWithId room:%@", roomId);
    ...
    [_turnClient requestServersWithCompletionHandler:^(NSArray *turnServers,
                                                       NSError *error) {
        if (error) {
            NSLog(@"connectToRoomWithId2 Error retrieving TURN servers: %@", error.localizedDescription);
        }
        ARDAppClient *strongSelf = weakSelf;
    ...
    [_roomServerClient joinRoomWithRoomId:roomId
                        ...
                        completionHandler:^(ARDJoinResponse *response, NSError *error) {
                            NSLog(@"connectToRoomWithId2 room:%@", roomId);
                            ...
                            NSLog(response.isInitiator ? @"isInitiator Yes" : @"isInitiator No");
                            NSLog(strongSelf.isCaller ? @"isCaller Yes" : @"isCaller No");
                            ...
                            NSLog(@"URL webSocketURL %@", response.webSocketURL);
                            NSLog(@"URL webSocketRestURL %@", response.webSocketRestURL);
...
- (BOOL)checkIfWeAreAlone{
    if(_isInitiator && !_isCaller){
        return YES;
    }
    return NO;
}

- (void)sendConnectedToBackendUsername: (NSString*)username
                       completionHandler:(void (^)(NSError *error))completionHandler{
    [_roomServerClient sendConnectedToBackendWithRoomId:_roomId clientId:_clientId username:username completionHandler:^(NSError *error) {
...
didReceiveMessage:(ARDSignalingMessage *)message {
    ...
    switch (message.type) {
        case kARDSignalingMessageTypeOffer:
            NSLog(@"didReceiveMessage kARDSignalingMessageTypeOffer");
        case kARDSignalingMessageTypeAnswer:
            NSLog(@"didReceiveMessage kARDSignalingMessageTypeAnswer");

            _hasReceivedSdp = YES;
            [_messageQueue insertObject:message atIndex:0];
            break;
        case kARDSignalingMessageTypeCandidate:
            NSLog(@"didReceiveMessage kARDSignalingMessageTypeCandidate");
        case kARDSignalingMessageTypeCandidateRemoval:
            NSLog(@"didReceiveMessage kARDSignalingMessageTypeCandidateRemoval");
            [_messageQueue addObject:message];
            break;
        case kARDSignalingMessageTypeBye:
            NSLog(@"didReceiveMessage kARDSignalingMessageTypeBye");
...
- (void)channel:(id<ARDSignalingChannel>)channel
 didChangeState:(ARDSignalingChannelState)state {
    ...
    NSError *socketError = [[NSError alloc] initWithDomain:kARDAppClientErrorDomain
                                                   code:kARDAppClientErrorSocketError
                                                   userInfo:nil];
    ...
    switch (state) {
        ...
        case kARDSignalingChannelStateError:
            ...
            [self.delegate appClient:self didError:socketError];
...
didGenerateIceCandidate:(RTCIceCandidate *)candidate {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"candidate %@", candidate.serverUrl);
        NSLog(@"candidate %@", candidate.sdpMid);
...
- (void)setMaxBitrateForPeerConnectionVideoSender {
    NSLog(@"setMaxBitrateForPeerConnectionVideoSender");
